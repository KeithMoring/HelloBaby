...
-(void)LoginTry{
    ...
    [BIDAFNetWork PostDataFromNet:strUrl InputParas:paras success:^(NSDictionary *resultDic) {
        [self setActivityViewOff];
        NSString *result=[resultDic objectForKey:@"LoginResult"];
        if([result isEqual:KEY_LOGIN_NOT_PASS]){
            NSLog(@"Password or  Username is not right");
        }else if([result isEqual:KEY_LOGIN_ERROR]){
            NSLog(@"Login Failed");
        }else{
            BIDAccount *account=[BIDAccount GetAccount];
            account.username=[paras objectForKey:@"name"];
